...
CREATE OR REPLACE FUNCTION fetch_collections_with_previews(
...
CREATE OR REPLACE FUNCTION fetch_ethscriptions_owned_with_listings_and_bids(
  address TEXT,
...
CREATE OR REPLACE FUNCTION fetch_ethscriptions_with_listings_and_bids(
  collection_slug text DEFAULT 'ethereum-phunks'
)
RETURNS TABLE(ethscription JSON) AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION fetch_events(
    p_limit INT,
