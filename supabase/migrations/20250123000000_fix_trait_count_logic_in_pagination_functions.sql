-- Fix trait counting logic to count individual trait values instead of attribute keys
-- This aligns the backend counting with the frontend logic

-- Drop and recreate the mainnet function
    ...
    AND (
        ...
        CASE
            WHEN trait_count_filter LIKE '%-%' AND trait_count_filter ~ '^[0-9]+-[0-9]+$' THEN
                -- Handle trait count range (e.g., "17-21") - FIXED FUNCTIONALITY
                (
                    SELECT COALESCE(SUM(
                        CASE
            ...
            ELSE
                -- Original exact match logic (backwards compatible) - FIXED
                (
                    SELECT COALESCE(SUM(
                        CASE
                            WHEN jsonb_typeof(a.values -> k) = 'array'
...
-- Drop and recreate the sepolia function
