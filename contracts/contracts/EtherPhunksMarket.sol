...
contract EtherPhunksMarket is ReentrancyGuard, Pausable, Ownable {
    ...
    function phunkNoLongerForSale(uint phunkIndex) public nonReentrant {
        ...
        if (phunksContract.ownerOf(phunkIndex) != msg.sender)
            revert("you are not the owner of this token");
        phunksOfferedForSale[phunkIndex] = Offer(
            false,
            phunkIndex,
            msg.sender,
            0,
    ...
    function buyPhunk(
        ...
        if (seller != phunksContract.ownerOf(phunkIndex))
            revert("seller no longer owner of phunk"); // Seller no longer owner of phunk
